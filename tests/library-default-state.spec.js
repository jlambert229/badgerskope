...
test.describe("Library — empty-state panel", () => {
  test("0-result filter combo shows the empty-state panel with a heading + Clear button", async ({ page }) => {
    await page.goto("/web/");
    await page.waitForSelector(".card", { timeout: 10_000 });
    ...
    // The heading rotates among three plain-English strings. Assert that
    // one matches without pinning to the specific copy.
    await expect(panel.locator(".empty-state__heading"))
      .toHaveText(/no matches|nothing here|empty/i);
