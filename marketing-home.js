  ...
  const renderLibrary = (filter) => {
    ...
    rowsEl.innerHTML = visible.map(f => `
      <a class="library-row" href="/web/#entry=${encodeURIComponent(f.slug)}" aria-label="${f.name}, evidence tier ${f.e}, safety tier ${f.s}, access tier ${f.a}">
        <div class="lib-name">
          <div class="lib-name-main">${f.name}</div>
          <div class="lib-name-aka">${f.aka}</div>
        </div>
        <div>${tierChip(f.e, 'Evidence')}</div>
        <div>${tierChip(f.s, 'Safety')}</div>
        <div>${tierChip(f.a, 'Access')}</div>
